...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const result = await githubCache.getOrSet(key, () => listOpenPulls(token, repo, page, perPage), PULLS_TTL);
        ...
        return NextResponse.json({ error: 'Failed to fetch pull requests', code: 'GITHUB_API_ERROR' }, { status: 502 });
    }
}
