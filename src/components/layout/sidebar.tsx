...
export function Sidebar({ mobileOpen, onCloseMobile }: SidebarProps) {
  ...
  const {
    selectedRepo, selectRepo, sidebarCollapsed,
    repoSearch, setRepoSearch, repoPage, setRepoPage,
  } = useDashboardStore();
  ...
  const { data: reposData, isLoading, error, refetch, isFetching } = useRepos(
    repoPage,
    30,
    activeSearch,
  );
                ...
                <ul className={styles.repoList} role="listbox" aria-label="Repository list">
                  {repos.map((repo: GitHubRepo) => (
                    <li key={repo.id}>
                      ...
                      >
                        <svg className={styles.repoIcon} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
