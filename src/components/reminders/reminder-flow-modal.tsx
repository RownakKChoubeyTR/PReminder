...
export function ReminderFlowModal() {
    ...
    const hasValidPendingEmail = missingEmailResults.some(r => EMAIL_RE.test((pendingEmails[r.login] ?? '').trim()));
    ...
    const handleSaveAndRetry = async () => {
        ...
        const filledEntries = missingEmailResults.filter(r => (pendingEmails[r.login] ?? '').trim().length > 0);
    ...
    const previewRecipient = eligibleRecipients[previewTabIndex] ?? eligibleRecipients[0] ?? 'reviewer';
            ...
            >
                <div className={styles.modalContent} ref={modalRef}>
                                        ...
                                        {missingEmailResults.length > 0 && (
                                            <div className={styles.missingEmailSection}>
                                                <div className={styles.missingEmailHeader}>
                                                    ...
                                                    <div>
                                                        <p className={styles.missingEmailTitle}>
                                                            Email address not found
                                                        </p>
                                                        <p className={styles.missingEmailSubtitle}>
                                                            Enter emails below to send the reminder and save for future
                                                            use.
                                                        </p>
                                                    </div>
                                                </div>
                                                    ...
                                                    {missingEmailResults.map(r => (
                                                        ...
                                                        >
                                                            ...
                                                            <div className={styles.missingEmailInfo}>
                                                                ...
                                                                <span className={styles.missingEmailGithub}>
                                                                    @{r.login}
                                                                </span>
                                                            </div>
                    ...
                    <div className={styles.footer}>
                        {step === 'channel' && (
                            <button type="button" className={styles.primaryButton} onClick={() => setStep('template')}>
