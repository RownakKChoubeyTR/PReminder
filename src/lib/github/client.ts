...
export async function listOrgRepos(
  ...
  const result = await getPage<GitHubRepo>(
    `/orgs/${ORG}/repos?type=all&sort=updated`,
    token,
    page,
    perPage,
  );
...
export async function listReviews(
  ...
  return getAll<GitHubReview>(
    `/repos/${ORG}/${encodeURIComponent(repo)}/pulls/${prNumber}/reviews`,
    token,
  );
}
...
export async function getPullRequest(
  ...
  try {
    return await get<GitHubPullRequest>(
      `/repos/${ORG}/${encodeURIComponent(repo)}/pulls/${prNumber}`,
      token,
    );
  } catch (err) {
...
export async function listTeamMembers(
  ...
  return getAll<GitHubTeamMember>(
...
export function isSamlError(err: unknown): boolean {
  return (
    ...
    err.status === 403 &&
    err.message.toLowerCase().includes('saml')
  );
}
