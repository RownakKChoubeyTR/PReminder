    ...
    for (let attempt = 0; attempt <= config.maxRetries; attempt++) {
        try {
            ...
            if (attempt < config.maxRetries) {
                ...
                dbLogger.error(`All ${config.maxRetries + 1} connection attempts failed. Last error: ${message}`);
                dbLogger.separator();
                throw new Error(
                    `[DB] Unable to connect after ${config.maxRetries + 1} attempts. ` + `Last error: ${message}`
                );
            }
        }
    }
