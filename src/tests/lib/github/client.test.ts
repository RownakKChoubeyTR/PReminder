...
describe('listOrgRepos', () => {
    ...
    it('should pass page and perPage params to the API', async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith(expect.stringMatching(/per_page=50&page=3/), expect.anything());
    });
...
describe('listReviews', () => {
    it('should fetch all reviews for a PR', async () => {
        const reviews = [{ id: 1, user: { login: 'reviewer1' }, state: 'APPROVED', submitted_at: '2025-01-01' }];
...
describe('searchOrgReposByName', () => {
    it('should search repos via GitHub Search API with org scoped query', async () => {
        ...
        expect(result.page).toBe(1);
        expect(result.hasNextPage).toBe(false);
        expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('/search/repositories'), expect.anything());
