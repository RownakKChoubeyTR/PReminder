...
describe('GET /api/reminders/cooldown', () => {
  ...
  it('returns 400 when recipients missing', async () => {
    const req = new NextRequest(
      'http://localhost/api/reminders/cooldown?prNumber=42&repo=org/repo',
    );
    const res = await GET(req);
  ...
  it('returns 400 when prNumber missing', async () => {
    const req = new NextRequest(
      'http://localhost/api/reminders/cooldown?recipients=alice&repo=org/repo',
    );
    const res = await GET(req);
  ...
  it('returns 400 when repo missing', async () => {
    const req = new NextRequest(
      'http://localhost/api/reminders/cooldown?recipients=alice&prNumber=42',
    );
    const res = await GET(req);
  ...
  it('returns 400 when recipients are empty after split', async () => {
    const req = new NextRequest(
      'http://localhost/api/reminders/cooldown?recipients=&prNumber=42&repo=org/repo',
    );
    const res = await GET(req);
